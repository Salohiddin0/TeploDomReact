  ...
  return (
    <section className='py-12 px-4 md:px-6'>
      <div className='max-w-7xl mx-auto'>
        <h2 className='text-2xl font-bold mb-16 text-center'>Наши преимущества</h2>
        ...
        <div className='grid gap-y-16 gap-x-8 md:grid-cols-3 place-items-center'>
          {advantages.slice(0, 3).map((advantage, index) => (
            <div key={index} className='flex flex-col items-center text-center max-w-[300px]'>
              <div className={`${advantage.bgColor} p-6 rounded-full flex items-center justify-center mb-6`}>
                <advantage.icon className={`w-8 h-8 ${advantage.iconColor}`} strokeWidth={1.5} />
              </div>
        ...
        <div className='flex justify-center gap-16 mt-16'>
          {advantages.slice(3).map((advantage, index) => (
            <div key={index + 3} className='flex flex-col items-center text-center max-w-[300px]'>
              <div className={`${advantage.bgColor} p-6 rounded-full flex items-center justify-center mb-6`}>
                <advantage.icon className={`w-8 h-8 ${advantage.iconColor}`} strokeWidth={1.5} />
              </div>
              <p className='text-gray-800 text-lg'>{advantage.text}</p>
